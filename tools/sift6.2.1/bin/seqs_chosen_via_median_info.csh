...
limit cputime 30m
	...
	$NCBI/psiblast -db $seq_database -query $query -out $query.out -outfmt 0 -num_iterations $iterations -evalue 0.0001 -num_descriptions 399 -num_alignments 399 

if ($status != 0) then
        echo "psiblast exiting because status not equal to 0"
        exit (-1)
endif
unlimit cputime
...
echo "$bindir/choose_seqs_via_psiblastseedmedian $query.unfiltered $tmpdir/$pid.clumped $query.selectedclumped 1.0 $tmpdir/$pid $median_threshold"

$bindir/choose_seqs_via_psiblastseedmedian $query.unfiltered $tmpdir/$pid.clumped $query.selectedclumped 1.0 $tmpdir/$pid $median_threshold >& /dev/null 

if ($status != 0) then
       echo "choose_seqs_via_psiblastseedmedian: exiting because stauts not equal to 0"
       exit (-1)
endif
unlimit cputime
rm -f $tmpdir/$pid.time.error
