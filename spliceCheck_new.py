			...
			for spliceai_key in spliceai_analyses:
				if float(entry.split(":")[0]) == (strand * float(spliceai_key.split(":")[0])):	# if splice ai finds an entry at the same position
					if mes_analyses[entry][2] == 1 and spliceai_analyses[spliceai_key][1] == 1 and int(mes_analyses[entry][1]) == int(spliceai_analyses[spliceai_key][0]):	# both SpliceAI and MES indicate strengthening, and referring to same site
						combined_count += 1
		if mes_count == 0 and combined_count == 0:
			aso_amenability = "It's very unlikely this variant is ASO-amenable. MES and SpliceAI analysis both do not predict any new sites being created/strengthened. {}".format(coding_impact)
		elif mes_count > 0 and combined_count == 0:
			if likely_count == 0:
				...
				aso_amenability = "It's likely that this variant is ASO-amenable. MES alone predicts a moderate/high probability that a new splice site is being created/strengthened. {}".format(coding_impact)
		else:
			aso_amenability = "It's very likely that this variant is ASO-amenable. MES and SpliceAI both predict that new sites are being created/strengthened. {}".format(coding_impact)
					...
					if mes_analyses[entry][2] == 0 or float(entry.split(":")[3]) < 4.00:	# The canonical site is predicted to be weakened
						...
						if mes_found:
							if len(beyond_results) == 0 and len(within_results) > 0:
								aso_amenability = "It's very unlikely this variant is ASO-amenable. Using MES, we analyzed that the canonical splice site is being weakened, while {} new splice site(s) are being created/strengthened within 10bp of the canonical site.".format(str(len(within_results)))
								...
								aso_amenability = "It's very likely this variant is ASO-amenable. Using MES, we analyzed that the canonical splice site is being weakened, while {} new splice site(s) are being created/strengthened beyond 10bp of the canonical site.".format(str(len(beyond_results)))
